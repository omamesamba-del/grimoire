...
I tried a lot of prompt management tools, but none of them felt quite right  Emissing features here, poor overview there. So I built grimoire with exactly the features I wanted: making prompt input for WebUI and ComfyUI easy, intuitive, and quick to edit.
...
The **Style Palette** lets you add decorative modifiers  Ecolors, materials, patterns, decorations  Eto any tag with a single click, without having to remember or type them manually.

**AI mode** connects to Ollama (local), Claude API, or OpenAI to generate SD-ready prompts from plain natural language  Ejust describe what you want.
...
**LoRA / Embedding / Checkpoint modes** can fetch metadata directly from CivitAI  Ethumbnails, trigger words, base model info, and more.

**Gen mode** gives you full control over generation settings, with quick resolution and aspect ratio presets built in.

---

![grimoire main screen](assets/grimoire002.gif)
...
![Library and Builder](assets/main.png)

### Style Palette
Apply style modifiers (Mod / Color / Material / Pattern / Deco) to any tag with a single click. Styles are prepended to your prompt automatically  Eno need to remember or type decorative words by hand.

![Style Palette](assets/grimoire003.gif)
...
![AI Assist](assets/ai-assist.png)

### Generation Settings
Configure checkpoint, VAE, sampler, steps, resolution, Hires.fix, and Refiner  Ethen send directly to WebUI or ComfyUI.

![Generation settings](assets/generation.png)

### Images Browser
Browse your output folder with a thumbnail grid. View PNG metadata (prompt, seed, generation params) inline.

![Images browser](assets/images.png)

### Tag Images
Attach a preview image to any tag  Eit appears as a thumbnail on the tag card. To register an image, drag and drop an image file directly onto a tag card in the library, or right-click a generated image in the Images Browser and select **Use as tag image**.

### CivitAI Integration
Fetch metadata for LoRA, Embedding, and Checkpoint files directly from CivitAI. Right-click any asset card and choose **Fetch Info (CivitAI)** to pull the cover image, trained words, base model, and description. Use **Fetch All** in LoRA / Embedding mode to batch-fetch metadata for all assets at once. A CivitAI API key can be set in **Settings ↁEAssets** to avoid rate limiting.
...
```
Restart WebUI, then set the WebUI URL in grimoire under **Settings ↁEGeneration** (default: `http://127.0.0.1:7860`).
...
```
Restart ComfyUI, then set the ComfyUI URL in grimoire under **Settings ↁEGeneration** (default: `http://127.0.0.1:8188`).

### How ComfyUI Mode Works

ComfyUI integration is more flexible than WebUI  Einstead of a single positive/negative pair, you can have **any number of named slots** that map directly to nodes in your workflow.

![ComfyUI slots panel](assets/comfy01.png)

Switch grimoire to **COMFY mode** using the toggle in the top-right corner. The builder panel changes to show **ComfyUI Slots**  Eone panel per Grimoire Slot node detected in your open workflow. Click a slot to make it the active target, then click tags from the library to add them there. Drag the `⠿` handle to reorder slots.

### Setting Up the Workflow

![ComfyUI workflow with Grimoire nodes](assets/comfy02.png)

The bridge provides three custom nodes, all found under the **PromptBuilder** category:

| Node | Description |
|------|-------------|
| **Grimoire Slot** | A named text slot. Set `slot_name` (e.g. `positive`, `chara`, `negative`) and connect the output to any STRING input  Etypically a CLIP Text Encode node. |
...
4. In grimoire, switch to COMFY mode  Ethe slots panel will auto-detect your slot names
...
Shortcuts are fully customizable under **Settings ↁEShortcuts**.
